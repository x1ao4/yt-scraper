...
while read id; do
    ...
    original_title=$(echo "$video_info" | jq -r '.title')
    if [[ "$original_title" != *'"'* && "$original_title" != *'“'* ]]; then
        skip_count=$((skip_count + 1))
        skip_reason="This video is not in the series"
        ...
        title=$(echo "$original_title" | sed -n -E 's/.*["“](.+)["”].*/\1/p')
        if [[ "$original_title" =~ (Part|Episode|Ep)[[:space:]]*([0-9]+) ]]; then
            title="$title - ${BASH_REMATCH[1]} ${BASH_REMATCH[2]}"
        elif [[ "$original_title" =~ Episode[[:space:]]*([[:alpha:]]+) ]]; then
            title="$title - Episode ${BASH_REMATCH[1]}"
        fi
    fi
    
    description=$(echo "$video_info" | jq -r '.description' | grep -v 'http' | sed '/^\s*$/d' | head -n 1)

    info_saved=true
    cover_saved=true

    upload_date=$(echo "$video_info" | $JQ -r '.upload_date')
    formatted_date_text=$(date -j -f "%Y%m%d" "$upload_date" "+%Y/%-m/%-d" 2>/dev/null)
    formatted_date_image=$(date -j -f "%Y%m%d" "$upload_date" "+%Y-%-m-%-d" 2>/dev/null)
    ...
    if [[ $? -ne 0 ]]; then
        ...
        if $download_info_temp; then
            printf "%s;%s;%s;%s\n" "$formatted_date_text" "$duration" "$title" "$description" >> "$output_file"
            if ! grep -q -- "$id" "$infos_archive_file" 2>/dev/null; then
                echo "$id" >> "$infos_archive_file"
            fi
        fi
    ...
    if $download_covers_temp; then
        ...
        title_filename=$(echo "$title" | sed 's/[\\/:"*?<>|“”]/ /g' | awk '{$1=$1};1')
        cover_output_file="$output_dir/$formatted_date_image $title_filename"
        if [[ -e "$cover_output_file.jpg" ]]; then
            i=1
            while [[ -e "$cover_output_file-$i.jpg" ]]; do
                i=$((i + 1))
            done
            cover_output_file="$output_dir/$formatted_date_image $title_filename-$i"
        fi
    ...
    if $info_saved && $cover_saved; then
        printf "\033[1A\033[K\033[1A\033[K\r"
        echo "Successfully saved video: $title"
        printf "\033[K\n\n"
